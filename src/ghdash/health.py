...
Deliberate design choice: "days since last commit," "is this stale," and
"open issue count" are reported as plain informational signals, not folded
into the health score. A finished personal project that hasn't been
touched in a year isn't unhealthy just because it's quiet -- staleness
only matters alongside missing hygiene basics, which is what the score
actually measures. Folding recency into the score would penalize exactly
the kind of project (done, stable, no reason to keep committing) that this
account has plenty of.
"""
...
class RepoHealth:
    ...
    is_stale: bool  # informational only -- see module docstring
...
def compute_repo_health(
    ...
    is_stale = days_since_last_commit is not None and days_since_last_commit >= stale_after_days
